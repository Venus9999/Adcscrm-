...
export const VisaApplicationModal: React.FC<VisaApplicationModalProps> = ({
  ...
  const handleSimulateDocUpload = (docReqName: string) => {
    const sampleFiles = [
      { name: `${docReqName.replace(/[^a-zA-Z0-9]/g, '_')}_Scan.pdf`, size: '1.8 MB' },
      { name: `${applicantName.replace(/[^a-zA-Z0-9]/g, '_')}_${docReqName.replace(/[^a-zA-Z0-9]/g, '_')}.pdf`, size: '2.4 MB' },
    ];
    const picked = sampleFiles[Math.floor(Math.random() * sampleFiles.length)];

    const newDoc: Omit<VisaUploadedDoc, 'id' | 'uploadedAt' | 'status'> = {
      docName: docReqName,
      docCategory: docReqName.toLowerCase().includes('passport')
        ? 'Passport'
        : docReqName.toLowerCase().includes('bank')
        ? 'Salary Slip'
        : docReqName.toLowerCase().includes('id')
        ? 'Emirates ID'
        : 'Other',
    ...
    setUploadedDocsList((prev) => {
      const filtered = prev.filter((d) => d.docName !== docReqName);
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {submitSuccess ? (
            ...
            <>
              ...
              {step === 3 && (
                <div className="space-y-5">
                  <div className="flex items-center justify-between p-3 rounded-xl bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 text-amber-900 dark:text-amber-200 text-xs">
                    <div className="flex items-center space-x-2">
                        ...
                        >
                          <div className="flex items-start space-x-3">
                            <div className={`mt-0.5 w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${
                              ...
                              {isUploaded ? <Check className="w-3.5 h-3.5" /> : idx + 1}
                            </div>
                            <div>
                              <p className="text-xs font-bold text-slate-900 dark:text-white">{req}</p>
                              <p className="text-[11px] text-slate-500">
                                {isUploaded ? 'Document attached & verified' : 'Required for consular dossier'}
                              </p>
                            </div>
                          </div>

                          <button
                            onClick={() => handleSimulateDocUpload(req)}
                            className={`px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center space-x-1.5 transition-all ${
                              isUploaded
                                ? 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300 hover:bg-emerald-200'
                                : 'bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-300 hover:bg-blue-100'
                            }`}
                          >

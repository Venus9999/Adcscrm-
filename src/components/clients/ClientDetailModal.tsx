        ...
        <div className="flex items-center gap-2 px-6 py-2.5 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30 overflow-x-auto text-xs font-semibold">
          ...
          <button
            onClick={() => {
              setActiveTab('gmail');
              if (isGmailConnected) {
                fetchMessages(undefined, `to:${client.email} OR from:${client.email}`);
              }
            }}
            className={`px-3.5 py-1.5 rounded-xl transition-all whitespace-nowrap flex items-center gap-1.5 ${
              activeTab === 'gmail'
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-6">
          ...
          {activeTab === 'gmail' && (
            <div className="space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-50 dark:bg-slate-800/40 p-4 rounded-2xl border border-slate-200 dark:border-slate-800">
                <div>
                  <h4 className="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2">
                    ...
                    <span>Gmail Communications ({client.email})</span>
                  </h4>
                  <p className="text-xs text-slate-500 mt-0.5">
                    Synchronized email messages, status notices, and direct dispatches via your connected Google account
                  </p>
                </div>
                  ...
                  {isGmailConnected ? (
                    <>
                      ...
                      >
                        ...
                        <span>Send Visa Status Email</span>
                      </button>
                    ...
                    <button
                      ...
                      className="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-xs font-bold flex items-center gap-1.5 shadow-xs cursor-pointer"
                    >
              ...
              {!isGmailConnected ? (
                <div className="p-8 text-center bg-slate-50 dark:bg-slate-800/20 border border-slate-200 dark:border-slate-800 rounded-2xl space-y-3">
                  <Mail className="w-8 h-8 mx-auto text-slate-400" />
                  <p className="text-xs font-semibold text-slate-700 dark:text-slate-300">
                    Connect your Gmail workspace to view email correspondence with {client.fullName}
                  </p>
                  <button
                    onClick={connectGmail}
                ...
                <div className="space-y-2.5">
                  {allGmailMessages.length === 0 ? (
                    <div className="p-6 text-center text-xs text-slate-500 italic bg-slate-50 dark:bg-slate-800/30 rounded-xl">
                      No emails currently found in inbox for {client.email}. Click "Compose Email" or "Send Visa Status Email" to initiate communication.
                    </div>
                  ) : (
                    allGmailMessages.slice(0, 10).map((msg) => (
                      <div
                        key={msg.id}

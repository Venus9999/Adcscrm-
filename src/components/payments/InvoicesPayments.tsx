...
export const InvoicesPayments: React.FC = () => {
  ...
  const handleOpenEditModal = (inv: Invoice) => {
    ...
    // Calculate verified payments from linked transactions
    const linkedTxs = (transactions || []).filter(
      (t) => t && t.invoiceId === inv.id && t.status !== 'cancelled' && t.status !== 'reversed' && t.status !== 'failed'
    );
    const netTxPaid = linkedTxs.length > 0
      ? Math.max(
          0,
          linkedTxs.filter((t) => !['refund', 'expense', 'withdrawal'].includes(t.type)).reduce((s, t) => s + (Number(t.amount) || 0), 0) -
          linkedTxs.filter((t) => t.type === 'refund').reduce((s, t) => s + (Number(t.amount) || 0), 0)
        )
      : 0;

    const initialAmountPaid = Math.max(Number(inv.amountPaid) || 0, netTxPaid);
    setEditAmountPaid(initialAmountPaid);
    setEditIssueDate(inv.issueDate);
    setEditDueDate(inv.dueDate);
    setEditPaymentMethod(inv.paymentMethod || (linkedTxs[0]?.paymentMethod as any) || 'Bank Transfer');
  ...
  const handleUpdateInvoiceSubmit = (e: React.FormEvent) => {
    ...
    // Check linked transactions to protect verified payments
    const linkedTxs = (transactions || []).filter(
      (t) => t && t.invoiceId === editInvId && t.status !== 'cancelled' && t.status !== 'reversed' && t.status !== 'failed'
    );
    const netTxPaid = linkedTxs.length > 0
      ? Math.max(
          0,
    ...
    const finalAmountPaid = Math.min(grandTotal, Math.max(editAmountPaid, netTxPaid));
    const balanceAmount = Math.max(0, grandTotal - finalAmountPaid);
    const resolvedStatus: Invoice['status'] =
      balanceAmount === 0 && grandTotal > 0 ? 'paid' : finalAmountPaid > 0 ? 'partially_paid' : editStatus;
                    ...
                    <tr key={inv.id} className="hover:bg-slate-50/70 dark:hover:bg-slate-800/40">
                      ...
                      <td className="py-3.5 px-4 text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          {/* Record Payment (Cash, Bank Transfer, Card) or Nomod Live Checkout */}
                          {inv.balanceAmount > 0 && (
                            <>
                              <button
                                onClick={() => {
                                  ...
                                  setPayNotes(`Settlement for Invoice #${inv.invoiceNumber}`);
                                  setShowPaymentModal(true);
                                }}
                                className="px-2.5 py-1 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-xs font-semibold flex items-center gap-1 shadow-xs transition-all cursor-pointer"
                                title="Record Cash, Bank Transfer, or Direct Settlement"
                              >
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    {activeInvoice && (() => {
                      const linkedTxs = (transactions || []).filter(
                        (t) => t && t.invoiceId === activeInvoice.id && t.status !== 'cancelled' && t.status !== 'reversed' && t.status !== 'failed'
                      );
                      const netTxPaid = linkedTxs.length > 0
                        ? Math.max(
                            0,
                        ...
                        : 0;
                      if (netTxPaid <= 0) return null;
                      return (
                        <button
                          type="button"
                          onClick={() => setEditAmountPaid(netTxPaid)}
                          className="text-[10px] text-blue-600 dark:text-blue-400 hover:underline font-medium"
                          title="Click to sync amount with verified linked payment transactions"
                        >
                          Sync Ledger ({netTxPaid.toLocaleString()} AED)
                        </button>
                      );
                    })()}
                  </div>
                  ...
                  {activeInvoice && (() => {
                    ...
                    return (
                      <div className="mt-1 text-[11px] text-slate-500 dark:text-slate-400 flex items-center justify-between">
                        <span>Verified in Ledger: <strong>{netTxPaid.toLocaleString()} AED</strong></span>
                        <span className="text-emerald-600 dark:text-emerald-400 font-medium">({linkedTxs.length} transaction{linkedTxs.length !== 1 ? 's' : ''})</span>
                      </div>
                    );
                  })()}
                </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 max-w-md w-full shadow-2xl animate-in fade-in">
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Invoice <strong className="font-mono">{activeInvoice.invoiceNumber}</strong> &bull; <strong>{activeInvoice.clientName}</strong>
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="mb-4 p-3 bg-slate-50 dark:bg-slate-800/80 rounded-xl border border-slate-200 dark:border-slate-700 space-y-1.5 text-xs">
              <div className="flex items-center justify-between text-slate-600 dark:text-slate-400">
                ...
                <span className="font-mono font-semibold text-slate-900 dark:text-white">AED {activeInvoice.grandTotal.toLocaleString()}</span>
              </div>
              <div className="flex items-center justify-between text-slate-600 dark:text-slate-400">
                <span>Previously Paid:</span>
                <span className="font-mono font-semibold text-emerald-600 dark:text-emerald-400">AED {(activeInvoice.amountPaid || 0).toLocaleString()}</span>
              </div>
              <div className="flex items-center justify-between text-slate-800 dark:text-slate-200 font-bold pt-1 border-t border-slate-200 dark:border-slate-700">
                ...
                <span className="font-mono text-amber-600 dark:text-amber-400">AED {(activeInvoice.balanceAmount || 0).toLocaleString()}</span>
              </div>
              {payAmount > 0 && (
                <div className="flex items-center justify-between text-[11px] pt-1 border-t border-dashed border-slate-200 dark:border-slate-700 text-blue-600 dark:text-blue-400 font-semibold">
                  ...
                  <span className="font-mono">AED {Math.max(0, (activeInvoice.balanceAmount || 0) - payAmount).toLocaleString()}</span>
                </div>
              )}
            </div>
            ...
            <form onSubmit={handleRecordPaymentSubmit} className="space-y-3">
              <div>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <div className="flex gap-1">
                    <button
                      type="button"
                      onClick={() => setPayAmount(activeInvoice.balanceAmount)}
                    ...
                    {activeInvoice.balanceAmount > 100 && (
                      <button
                        type="button"
                        onClick={() => setPayAmount(Math.round(activeInvoice.balanceAmount / 2))}
              ...
              <div>
                ...
                >
                  <option value="Cash">Cash (Counter / Receipt Voucher)</option>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. CASH-REC-001 or ENBD-WT-99201"
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Full counter settlement receipt issued"
                  className="w-full p-2.5 bg-slate-50 dark:bg-slate-800 rounded-xl text-xs border border-slate-200 dark:border-slate-700"
                />
              </div>
              ...
              <div className="flex justify-end gap-2 pt-2 border-t border-slate-200 dark:border-slate-800">
                <button
                  type="button"
                  onClick={() => setShowPaymentModal(false)}
                  className="px-4 py-2 bg-slate-100 dark:bg-slate-800 rounded-xl text-xs font-semibold cursor-pointer"
                >
                  Cancel
                </button>
                <button
                ...
                >
                  ...
                  <span>Confirm & Reduce Invoice Balance</span>
                </button>
              </div>
            </form>
          </div>
            ...
            <form onSubmit={handleSaveEditTx} className="space-y-3.5 pt-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    value={editTxData.paymentMethod ?? 'Cash'}
